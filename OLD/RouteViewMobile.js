...
return;
    ...
    function startup( ) {
		...
		document.getElementById('div_panorama').ondblclick = cb_panorama_dblclick;
...
//      console.log( "d=" + d + " - " + polyline.GetIndexAtDistance(d) + " / " + bearing + " - " + p.G + " , " + p.K);

        dijit.byId('id_input_route').set( 'value', curr_dist, false );

        sv.getPanoramaByLocation( new google.maps.LatLng( p.G, p.K ), 0, function( data, status ) {
        	...
        	if ( status === google.maps.StreetViewStatus.OK ) {
                console.log( div_panorama_width + 'x' + div_panorama_height );
                marker.setPosition( new google.maps.LatLng( p.G, p.K ) );
                var img = document.getElementById("div_panorama");
                img.src = 'http://maps.googleapis.com/maps/api/streetview' + 
                	'?size=' + div_panorama_width + 'x' + div_panorama_height + 
                	'&location=' + p.G  + ',' + p.K  +
                	'&sensor=false' + 
                	'&heading=' + bearing;
        		...
        		console.log( "Unknown Error! - d=" + d + " - " + polyline.GetIndexAtDistance(d) + " / " + bearing + " - " + p.G + " , " + p.K);
        	}
        	else if ( status === google.maps.StreetViewStatus.ZERO_RESULTS ) {
        		console.log( "Zero Results! - d=" + d + " - " + polyline.GetIndexAtDistance(d) + " / " + bearing + " - " + p.G + " , " + p.K);
                marker.setPosition( new google.maps.LatLng( p.G, p.K ) );
                var img = document.getElementById("div_panorama");
                img.src = 'http://maps.googleapis.com/maps/api/streetview' + 
            		'?size=' + div_panorama_width + 'x' + div_panorama_height + 
...
//     		domGeom.setContentSize( dom.byId("div_panorama"), {w:w_body, h:h_body-h_controls} );
     		...
     		div_panorama_width  = w_body;
     		div_panorama_height = h_body-h_controls;
    ...
    return {
        ...
        do_split_unsplit:  	function( ) { do_split_unsplit(); },
